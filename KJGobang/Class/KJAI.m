...
@implementation KJPointData

- (id)init {
    
    self = [super init];
    if (self) {
        self.p = [[KJPoint alloc] initPointWith:-1 y:-1];
        self.count = 0;
    }
...
- (id)initWithPoint:(KJPoint *)point count:(int)ncount {
...
//    NSLog(@"is Step emergent");
    ...
    KJPoint* check = [[KJPoint alloc] initPointWith:pp.x y:pp.y];
    if (![self checkPoint:check]) {
        return FALSE;
    } else {
        
        KJPoint *test = [[KJPoint alloc] initPointWith:check.x y:check.y]; // 开始进行寻找的点
        KJPoint *testR = [[KJPoint alloc] initPointWith:check.x y:check.y]; // 遍历点的左侧的点
        KJPoint *testL = [[KJPoint alloc] initPointWith:check.x y:check.y]; // 遍历点的右侧的点
        
        testR = [[KJPoint alloc] initPointWith:check.x + 1 y:check.y]; // 开始遍历点的正右侧
        testL = [[KJPoint alloc] initPointWith:check.x - 1 y:check.y]; // 开始遍历的点的正左侧
        
        for(int hi = 1; (test.x+hi < kBoardSize) && ([self.curBoard[(int)testR.x][(int)testR.y] integerValue] == xType); hi++) //在相应点向右侧进行遍历，寻找最长的连续距离
            testR = [[KJPoint alloc] initPointWith:testR.x + 1 y:testR.y];
        for(int hii = 1; (test.x-hii >= 0) && ([self.curBoard[(int)testL.x][(int)testL.y] integerValue] == xType); hii++) //在相应点向左侧进行遍历，寻找最长的连续距离
            testL = [[KJPoint alloc] initPointWith:testR.x - 1 y:testR.y];
        ...
        testR = [[KJPoint alloc] initPointWith:check.x y:check.y + 1]; // 开始遍历点的正下方
        testL = [[KJPoint alloc] initPointWith:check.x y:check.y - 1]; // 开始遍历的点的正上方
...
- (KJPoint *)nextStep:(OccupyType)xType num:(int)num thre:(int)threshold x:(int)startX y:(int)startY {
    
    KJPoint *search = [[KJPoint alloc] initPointWith:startX y:startY];
    KJPoint * hPoint;
    KJPoint * vPoint;
    KJPoint * rPoint;
    KJPoint * lPoint;
    ...
    KJPoint * invalid = [[KJPoint alloc] init];
    return invalid;
}

- (KJPoint *)horizontal:(KJPoint *)pp type:(OccupyType)xType num:(int)num thre:(int)threshold {
    ...
    int count = 0;
    KJPoint *solution = [[KJPoint alloc] init];
...
- (KJPoint *)vertical:(KJPoint *)pp type:(OccupyType)xType num:(int)num thre:(int)threshold {
...
- (KJPoint *)rightDown:(KJPoint *)pp type:(OccupyType)xType num:(int)num thre:(int)threshold {
...
- (KJPoint *)leftDown:(KJPoint *)pp type:(OccupyType)xType num:(int)num thre:(int)threshold {
...
- (KJPoint *)getNextPoint:(KJPoint *)pp {
    
    KJPoint *result = [[KJPoint alloc] init];
    result.x = pp.x;
    result.y = pp.y;
...
+ (KJPoint *)geablog:(NSMutableArray *)board type:(OccupyType)type {
    ...
    KJPoint *calibur = [[self class] SeraphTheGreat:board type:type];
    return calibur;
}

+ (KJPoint *)SeraphTheGreat:(NSMutableArray *)board type:(OccupyType)myType {
    
    KJOmni *omniknight = [[KJOmni alloc] init];
    if(myType == OccupyTypeUser)

        omniknight = [[KJOmni alloc] initWithArr:board opp:OccupyTypeAI my:OccupyTypeUser];
    else

        omniknight = [[KJOmni alloc] initWithArr:board opp:OccupyTypeUser my:OccupyTypeAI];
    KJPoint *calibur;
...
//  如果什么都没有则返回不可用的点
    KJPoint *sad = [[KJPoint alloc] initPointWith:(kBoardSize+1)/2 y:(kBoardSize+1)/2];
    return sad;
}
